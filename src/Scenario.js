    ...
    <span>
      {out.reduce((acc, value, i) => {
        for (let k = 0; k < value; k++) {
          acc.push(
            <FontAwesomeIcon
              key={`${['faChild', 'faBlind'][i]}_${k}`}
              icon={[faChild, faBlind][i]}
              style={{ width: '20px', height: '20px' }}
            />
          );
        }
        if (value === 0) acc.push('none');
        if (i === 0) acc.push('|');
        return acc;
      }, [])}
    </span>
...
export const DilemmaMaker = (mode = 'random', n = 2) => {
  ...
  const features = index.map(i =>
    featureNames.reduce((p, d) => {
      p[d] = distros[d]();
      return p;
    }, {})
  );
...
export class DataGenerator {
  ...
  next() {
    const out =
      this.loc + 1 >= this.source.length
        ? null
        : [this.source[this.loc], this.source[this.loc + 1]].map(
            d => new Person(featureNames, d)
          );
    this.loc += 2;
