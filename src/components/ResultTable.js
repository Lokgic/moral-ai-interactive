...
const getURL =
  process.env.NODE_ENV === 'development'
    ? 'http://localhost:5000/get-stat'
    : 'https://moralai.herokuapp.com/get-stat';
      ...
      .then(d => {
        ...
        const stat = d.map((d, i) => {
          ...
          const decision =
            randomChoices[i] === 1
              ? 'random'
              : [`${features[i][0].name}(L)`, `${features[i][1].name}(R)`][
                  labels[i]
                ];
      ...
      <div>
        ...
        <ResultTableEl>
          <tbody>
            <ResultTableRow>
              {columns.map(d => (
                <ResultTableHead key={`${d}_column`}>{d}</ResultTableHead>
              ))}
            </ResultTableRow>
            ...
            {rows.map((row, ri) => {
              ...
              return (
                <ResultTableRow key={`${ri}row`}>
                  {subI === 0 ? (
                    <ResultTableCell type={type} rowSpan="2">
                      {mainI + 1}
                    </ResultTableCell>
                  ) : null}
                  {innerRowFeat.map(d => (
                    <ResultTableCell type={type} key={`${d}_feat_${ri}`}>
                      {row[d]}
                    </ResultTableCell>
                  ))}
                  {subI === 0
                    ? [
                        ...
                        <ResultTableCell
                          ...
                          rowSpan="2"
                        >
                          {statReady ? stat[mainI][h] : loading}
                        </ResultTableCell>
                      ))
                    : null}
                </ResultTableRow>
